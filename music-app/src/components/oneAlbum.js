...
function OneAlbum({match}) {
    ...
    const fetchHomeData = async () => { 
        ...
        const arrSongsLink = [...new Set(album.map((item)=>(`${item.youtube_link.slice(30)}`)))]
        ...
        <li className='songsList' key={item}>
            ...
            <Link to={`/songs/${arrSongsId[i]}?album=${match.params.id}`}>{item}</Link>
            <span>{album ? album[0].name:'loading...'}</span>
            <span>{arrSongsTime ? arrSongsTime[i].slice(3):'loading...'}</span>
        </li>
        ))
        const arrArtistsTitle = [...new Set(album.map((item)=>(`${item.artistName}`)))]
        ...
        <div className='artistsList' key={item}>
            ...
            <div><Link to={`/artists/${arrArtistsId[i]}`}>{item}</Link></div>
        </div>
        ))     
    ...
    return (
        <div>
            ...
            {album.length>0 ? <img src={album[0].cover_img} alt='Views' width="400" height="500"/>:<div>Loading...</div>}
            <div>
            <h4>Songs:</h4>
            <ul>
            {finalSongsArr}
