...
function OneSong({match, location}) {
    ...
    const sideListId = playlist.id? playlist:artist.id? artist:album.id? album:topSongs.id? topSongs:undefined; 
    console.log(sideListId)

    function showLyrics() {
        displayLyrics === 'none' ? setDisplayLyrics('block'):setDisplayLyrics('none')
    }

    const fetchHomeData = async () => {
        const sideListFetch = sideListId? sideListId.table === 'songs'? 
        await fetch(`/api/top_${sideListId.table}`):await fetch(`/api/${sideListId.table}/${sideListId.id}`):[];
        ...
        const arrSongsLink = [...new Set(sideList.map((item)=>(`${item.youtube_link.slice(30)}`)))]
        const arrSongsTime = [...new Set(sideList.map((item)=>(`${item.time}`)))]
        const arrSongsName = [...new Set(sideList.map((item)=>(`${item.name}`)))]
        const arrSongsTrack = [...new Set(sideList.map((item)=>(`${item.track_number}`)))]
        const arrId = [...new Set(sideList.map((item)=>item.songsId))]
        const finalArr = arr.map((item,i) => {
            return (
            ...
            <NavLink activeStyle={{
                fontWeight: "bold",
                color: "red"
              }} to={`/songs/${sideListId.table=== "songs"? sideList[i].id:arrId[i]}?${sideListId.table=== "playlist"? 
              sideListId.table:sideListId.table=== "artists"? "artist":
              sideListId.table=== "albums"? "album":'top_songs'}=${sideListId.id}`}>{item}</NavLink>
              <span>{arrSongsName[0]}</span>
              <span>{sideListId.table=== "playlist"? `Song Number: ${arrSongsTrack[i]}`:
              sideListId.table=== "songs"? sideList[i].time.slice(3):arrSongsTime[i].slice(3)}</span>
              </li>
            )}).slice(0,9)
    ...
    return (
        <div>
            ...
            <button className='lyricButton' onClick={()=> showLyrics()}>{displayLyrics === 'none' ? 'Show Lyrics':'Hide Lyrics'}</button>
            </div>
            </div>
            {song.map(item=> (
