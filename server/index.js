...
app.get('/api/:tableName/:id', (req,res)=>{
    ...
    table === 'artists'? `SELECT artists.*, songs.*,songs.id AS songsId,albums.cover_img AS albumCover,albums.name AS albumName,albums.id AS albumsId FROM artists JOIN songs 
    ...
    WHERE artists.id = ${id};`:
    table === 'albums'? `SELECT albums.*,artists.id AS artistId,artists.name AS artistName,artists.cover_img AS artistImg,songs.title,songs.youtube_link,songs.time , songs.id AS songsId FROM albums
    JOIN albums_and_artist
    ...
    WHERE albums.id =${id};`:`SELECT *,albums.name AS albumName,artists.name AS artistName FROM songs
    JOIN albums
